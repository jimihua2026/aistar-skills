...
粗看后读取本包 [候选边界与连续性复核](boundary-review.md)，按其局部基线、变化机制及短镜头规则记录候选证据。此阶段只记录观察与候选，接受/拒绝/暂定结论由镜头分析阶段写入。

1. 使用分析代理（无代理时使用原视频）完整浏览一次，记录场景、主体、显著剪辑/转场、主动作、摄影机运动与节奏。
2. 建立带实际时间锚点的时间轴；秒级粗定位后仅对疑似边界加密。硬切、淡入淡出、叠化、遮挡/物体转场及显著画面结构改变均标为候选边界。
