...
| `shot-deconstruction` | 镜头解构 | 1.3.0 | 拆解参考 AI 视频镜头，并为每个镜头生成可独立使用的纯文字复刻提示词。 |
| `video-reproduction` | 视频复刻 | 1.1.0 | 保留参考视频的构图、动作、运镜与转场，替换主体、元素或画风，并交付关键状态图指令、生成任务与验收规则。 |
...
镜头解构与视频复刻各自包含完整的候选边界复核协议：比较附近画面变化与前后连续性，区分剪辑镜头、镜内阶段和持续转场。两者可分别安装分发，不依赖彼此，也不新增 PySceneDetect 等切镜检测库。
