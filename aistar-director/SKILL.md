...
metadata:
  version: "1.1.1"
  compatibility: 支持能够读取 Markdown 与相对路径资源、处理用户提供的文本和媒体并写入工作区的 AI Agent。缺少特定模态或文件能力时，只完成可验证部分并明确降级。
---
...
1. 本 Skill 不依赖固定产品名或工具名。把“读取文档、查看图片、检索资料、写入文件”等能力映射到宿主 Agent 提供的等价工具。
2. 所有相对路径均以当前 Skill 目录为基准解析；只有执行到对应阶段时才读取相关 `references/`、`templates/` 或 `examples/` 文件。
3. 宿主不支持自动 Skill 发现时，将本 `SKILL.md` 作为项目或系统指令加载，并同时提供其引用资源。
4. 缺少某种输入能力时，请求用户转换格式或只完成能够验证的部分；不得臆造未读取的文档、图片、音频或工具结果。
5. 网络访问、软件安装、外部发送和文件覆盖均遵循宿主平台的权限机制，不因本 Skill 被调用而获得额外授权。

---

## 版本管理

当前 Skill 版本以 `SKILL.md` frontmatter 中的 `metadata.version` 为唯一判断依据，本版为 `1.1.1`。版本号采用 `主版本.次版本.修订版本`：不兼容的流程或结构变化升级主版本，向后兼容的新能力升级次版本，修复与文字澄清升级修订版本。
...
本 `1.1.1` 版本沿用 0822 定稿工作流，并增加跨 Agent 宿主适配约定，至少必须能够稳定完成：
